...
";
            ...
            Approvals.Verify(LtInfo.Common.DataTableExtensions.TableToHumanReadableString(ExecAdHocSql(sql)));
        }
...
begin
    ...
    set @sqlQuery = 'select
        @allFeatureCountOutput = (select count(*) from TABLE_SCHEMA.TABLE_NAME where COLUMN_NAME is not null),
        @wrongSridFeatureCountOutput = (select count(*) from TABLE_SCHEMA.TABLE_NAME where COLUMN_NAME.STSrid != {FirmaWebConfiguration.GeoSpatialReferenceID})';
    ...
    set @sqlQueryToTroubleshoot = 'select *, COLUMN_NAME.STAsText() as COLUMN_NAMEAsText, ActualSrID COLUMN_NAME.STSrid, {FirmaWebConfiguration.GeoSpatialReferenceID} as ExpectedSrID from TABLE_SCHEMA.TABLE_NAME where COLUMN_NAME.STSrid != {FirmaWebConfiguration.GeoSpatialReferenceID}';
