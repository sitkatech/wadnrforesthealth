...
{
    public class FundSourceAllocationBudgetLineItemGridSpec : GridSpec<Models.FundSourceAllocationBudgetLineItemForGrid>
    {
        public FundSourceAllocationBudgetLineItemGridSpec()
        {
            ObjectNameSingular = $"{Models.FieldDefinition.FundSourceAllocationBudgetLineItem.GetFieldDefinitionLabel()}";
            ObjectNamePlural = $"{Models.FieldDefinition.FundSourceAllocationBudgetLineItem.GetFieldDefinitionLabelPluralized()}";
            SaveFiltersInCookie = true;

            //CustomExcelDownloadUrl = SitkaRoute<FundSourceController>.BuildUrlFromExpression(tc => tc.FundSourcesExcelDownload());

            Add(Models.FieldDefinition.FundSourceAllocationName.ToGridHeaderString(), x => UrlTemplate.MakeHrefString(x.FundSourceAllocation.GetDetailUrl(), x.FundSourceAllocation.FundSourceAllocationName), 225,AgGridColumnFilterType.Text);
            ...
            // 9/22/20 TK - If showing these in the grid, I recommend adding them back to the TotalAmount in the class FundSourceAllocationBudgetLineItemForGrid
