...
dhtmlXGridObject.prototype.toPDF = function (url, mode, header, footer, rows, target) {
	...
	function* xml_body() {
		...
		if (rows)
			for (var i = 0; i < rows.length; i++) {
				 yield xml_row(grid.getRowIndex(rows[i]));
				//console.log('in xml_body, yielding : ', xml);
				//yield xml;
			}
		else
			for (var i = 0; i < grid.getRowsNum(); i++) {
	...
	//d.innerHTML = '<form id="' + uid + '" method="post" action="' + url + '" accept-charset="utf-8"  enctype="application/x-www-form-urlencoded"' + this.target + '><input type="hidden" name="grid_xml" id="grid_xml"/> </form>';
	encodedHeader = encodeURIComponent(xml_top(mode).replace("\u2013", "-"));
	//document.getElementById(uid).firstChild.value = encodedHeader.length.toString(16) + "\r\n" + encodedHeader + "\r\n";
	...
	const stream = new ReadableStream({
		async start(controller) {
			console.log("hello im in the stream");
			
			controller.enqueue(encodedHeader);
			console.log("here 0!");

			const iterator = xml_body();
			console.log("here 1!");

			//for (let val of iterator) {
			//	//console.log("Encoding and enqueing: ", val);
			//	var nextChunk = encodeURIComponent(val);
			//	controller.enqueue(nextChunk);
			//}
			console.log("here 2!");
			var footer = encodeURIComponent(xml_end());
			controller.enqueue(footer);
			controller.close();
		}
	}).pipeThrough(new TextEncoderStream());
	//debugger
	console.log("stream looks like: ", stream);
	//var footer = encodeURIComponent(xml_end());
	//document.getElementById(uid).firstChild.value = footer.length.toString(16) + "\r\n" + footer + "0\r\n\r\n";
	//document.getElementById(uid).submit();
